...
*/
	class GebruikersPage extends Core implements iPage {
		...
		private function getData(){
			...
			$sql='SELECT username,email,uuid FROM `tb_users` WHERE status = 0 OR status = 1 ORDER BY username';
...
HTML;
		...
		private function read() {
			...
			$sql = "SELECT * FROM tb_users WHERE uuid = ? AND status = ?";
			$aData = array(PARAM, '1');
				...
				<fieldset>
					<legend>Voeg een nieuwe gebruiker toe</legend>
						<form action="$url" enctype="multipart/formdata" method="post">
							...
							<label>Status</label>
							<input type="text" name="status" id="" value="$status" placeholder="Status" />
...
HTML;
		...
		private function processFormEditUser() {
			...
			$status     = $_POST['status'];
			// create insert query with all info above
			$sql = "UPDATE tb_users SET username='$username', password='$password',email='$email',role='$role',status='$status' WHERE uuid = ?";
			$aData = array(PARAM);
